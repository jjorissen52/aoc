...
export const SyntaxRunner = new CodeRunner((input: string) => {
...
export const AutocompleteRunner = new CodeRunner((input: string) => {
  const lines = input.split("\n").filter((line) => !!line);
      ...
      Object.keys(chunked_line)
        ...
        .forEach((depth) => {
          ...
          if (incomplete(ctrl_indexes)) {
            additions += partners[line[ctrl_indexes[ctrl_indexes.length - 1]]];
          }
        });
      return additions;
    });
